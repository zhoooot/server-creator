...
export class ReportController {
    ...
    @Get('/:uid/:status/')
    async getReportOfUserByStatus(@Param('uid') uid: string, @Param('status') status: boolean) {
        return await this.reportService.getReportOfUserByStatus(uid, status);
    }

    @Post('/')
    async createReport(@Body('user_id') uid: string, @Body('quiz_id') qid: string, @Body('detail') detail: string) {
